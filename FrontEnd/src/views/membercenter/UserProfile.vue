                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-12">
                            ...
                            <input type="tel" class="form-control" id="phone" v-model="store.getProfile.userPhone"
                                :class="{ 'is-invalid': validationErrors.userPhone }" required>
                            <div class="invalid-feedback">
                                請輸入有效的電話號碼
                            </div>
                        </div>
...
export default {
    ...
    setup() {
        ...
        onMounted(() => {
            ...
            store.loadMemberData(memberId)
            initFormData()//初始化撈資料
            console.log('初始化完'+store.userEmail)
        })
        ...
        const initFormData = async () => {
            try {
                ...
                // 如果地址存在，解析城市和地區
                if (store.getProfile.userAddress) {
                    const addressParts = parseAddress(store.getProfile.userAddress)
                    if (addressParts) {
                        selectedCity.value = addressParts.city
                        selectedDistrict.value = addressParts.district
                    }
                }

                // 更新 store 中的會員資料
                // store.updateProfile(response.data)
            } catch (error) {
        ...
        const validateForm = () => {
            ...
            if (!formData.userPhone) {
                validationErrors.value.userPhone = '請輸入電話號碼'
                isValid = false
            }
